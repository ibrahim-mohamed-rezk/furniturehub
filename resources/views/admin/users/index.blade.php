                ...
                <header class="card-header">
                    <div class="row align-items-center">
                        <div class="col-md-2 col-12 me-auto mb-md-0 mb-3">
                            <input user="text" name="name" placeholder="{{ __($routes . '.name') }}"
                                class="form-control" value="{{ $request->name }}">
                        </div>
                        <div class="col-md-2 col-12 me-auto mb-md-0 mb-3">
                            <input user="text" name="email" placeholder="{{ __($routes . '.email') }}"
                                class="form-control" value="{{ $request->email }}">
                        </div>
                        <div class="col-md-2 col-12 me-auto mb-md-0 mb-3">
                            <input user="number" name="phone" placeholder="{{ __($routes . '.phone') }}"
                        ...
                        <div class="col-md-2 col-6">
                            <button user="submit" class="btn btn-primary form-control" style="color:white">
                ...
                @foreach ($users as $key => $row)
                    <article class="itemlist">
                        <div class="row align-items-center">
                            ...
                            @if (!$row->deleted_at)
                                @if ($row->checkTrack($row->id) !== null)
                                    <div class="col-lg-2 col-sm-4 col-8 flex-grow-1 col-name">
                                        <div class="info">
                                            <h6 class="mb-0 text-danger" >{{ $row->checkTrack($row->id) }}</h6>
                                        </div>
                                    </div>
                                @else
                                    <div class="col-lg-2 col-sm-2 col-4 col-action text-end">
                                        ...
                                        <a class="btn btn-sm font-sm rounded btn-brand mr-5"
                                            href="{{ route('users.track', $row->id) }}"><i
                                                class="material-icons md-edit"></i>
                                            {{ __('dashboard.track') }}</a>
                                    </div>
                                @endif
                            @endif
                            ...
                            <div class="col-lg-2 col-sm-2 col-4 col-action text-end">
                                ...
                                @endif
                                <a class="btn btn-sm font-sm btn-light rounded" onclick="deleteRow(this)"
                                    href="{{ route($routes . '.destroy', $row->id) }}#"><i
                                        class="material-icons md-delete_forever"></i>
                                    {{ __('dashboard.' . $delete_button) }}</a>
                            </div>
                        </div>
                        <!-- row .//-->
                    </article>
